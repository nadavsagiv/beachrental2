...
export default function SnorkelsTab() {
  ...
  const filteredRentals = rentals.filter(rental => {
    if (rental.type !== 'SNORKEL') return false;
    switch (filter) {
      case 'active':
    ...
    <Box>
          ...
          <Grid item xs={12} sm={6} md={4} key={rental.id}>
            <Card>
              <CardContent>
                ...
                {!rental.endTime && (
                  <Timer startTime={rental.startTime} rentalType="SNORKEL" />
                )}
                {rental.endTime && (
                  <Typography color="text.secondary">
      ...
      <Dialog open={isNewRentalDialogOpen} onClose={() => setNewRentalDialogOpen(false)}>
        <form onSubmit={handleNewRental}>
          <DialogTitle>השכרת שנורקל חדש</DialogTitle>
          <DialogContent>
            <TextField
              label="מספר אנשים"
              name="numPeople"
